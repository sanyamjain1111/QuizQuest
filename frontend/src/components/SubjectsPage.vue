<template>
  <div class="admin-dashboard-container">
    ...
    <div class="dashboard-header">
      <h1 class="dashboard-title">Subjects Management</h1>
    </div>
    
    <div class="main-content">
      <!-- Add Subject Button -->
      <div class="action-buttons">
        <button class="add-quiz-btn" @click="navigateToAddSubject">
      ...
      <div class="quiz-container">
        <div class="quiz-count" v-if="subjects.length > 0">
          <span class="material-icons">menu_book</span>
          <span>{{ filteredSubjects.length }} {{ filteredSubjects.length === 1 ? 'Subject' : 'Subjects' }} Available</span>
        </div>
        <div v-if="subjects.length > 0" class="quiz-list">
          <div v-for="(subject, index) in filteredSubjects" :key="subject.subject_id" class="quiz-item">
            <div class="quiz-badge">{{ index + 1 }}</div>
            <div class="quiz-item-header">
              <div class="quiz-item-title">
                <span class="quiz-id">Subject #{{ subject.subject_id }}</span>
                <span class="chapter-name">{{ subject.subject_name }}</span>
              </div>
              <div class="quiz-item-meta">
                ...
                <div class="meta-item">
                  ...
                  <span>{{ getChapterCount(subject.subject_id) }} Chapters</span>
                </div>
              </div>
            </div>
            <div class="quiz-item-actions">
              <button class="action-btn edit-btn" @click="editSubject(subject.subject_id)">
                <span class="material-icons">edit</span>
                Edit
              </button>
              <button class="action-btn delete-btn" @click="confirmDelete(subject.subject_id)">
        ...
        <div v-if="subjects.length === 0" class="empty-state">
          ...
          <p>Create your first subject by clicking the "Add New Subject" button above</p>
        </div>
        
        <!-- Empty state when no subjects match search -->
        <div v-if="subjects.length > 0 && filteredSubjects.length === 0" class="empty-state">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            Are you sure you want to delete this subject? This action cannot be undone and will also delete all associated chapters and questions.
          </div>
...
export default {
  ...
  methods: {
    ...
    navigateToAddSubject() {
      this.$router.push('/admin/subjects/add');
    },
    editSubject(subjectId) {
      this.$router.push(`/admin/subjects/edit/${subjectId}`);
    },
    ...
    async deleteSubject() {
      ...
      try {
        ...
        await subjectService.deleteSubject(this.subjectToDelete);
        
        // Remove the subject from the list
        this.subjects = this.subjects.filter(subject => subject.subject_id !== this.subjectToDelete);
        
        // Close the modal and reset subjectToDelete
        if (this.deleteModal) {
        ...
        if (this.deleteModal && this.subjectToDelete) {
          this.deleteModal.hide();
          this.subjectToDelete = null;
        }
      }
